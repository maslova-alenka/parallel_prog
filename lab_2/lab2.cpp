...
void write_result(const std::string& filepath, const std::vector<std::vector<int>>& matrix_3, int size, double duration) {
...
int main() {
    ...
    std::ofstream result("C:/Repository/lab_parallel_prog/lab_2/result.txt");
    ...
    for (int size : sizes) {
        ...
        write_result(result_file, matrix_3, size, end - start);
        result << "Size: " << size << " Time: " << (end - start) << " seconds" << std::endl;
    }
    result.close();
