...
const std::string BASE_DIR = "C:/Repository/lab_parallel_prog/lab3/matrix";
const std::string RES_DIR = "C:/Repository/lab_parallel_prog/lab3/result";
...
void save_matrix(const std::vector<int>& matrix, int size, const std::string& filename) {
...
int main(int argc, char** argv) {
    ...
    for (int size : sizes) {
        std::vector<int> A, B, C(size * size);
        ...
        if (rank == ROOT_RANK) {
            ...
            save_matrix(A, size, file1);
            save_matrix(B, size, file2);
        }
        else {
            A.resize(size * size);
            B.resize(size * size);
        ...
        if (rank == ROOT_RANK) {
            ...
            save_matrix(C, size, result_file);
        }
    ...
    if (rank == ROOT_RANK) {
        std::ofstream summary(RES_DIR + "/results_summary.csv");
